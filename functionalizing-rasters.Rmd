...
plot(chm.htclass,
     ...
     legend=FALSE)
# to remove oversized box, can add axes=FALSE and box=FALSE
# but then have to put axes back in

chm.htclass

## playing around with adding outside line and axes
# plot(extent(chm.htclass), add=TRUE, lwd=2)  # adds outline back to plot
# # working on putting axes back in, needs some work
# axis(2, at=axTicks(2), pos=(par()$usr[1]+140), labels=paste(axTicks(2)))
# axis(1, at=axTicks(1), pos=(par()$usr[3]), labels=paste(axTicks(1)))
